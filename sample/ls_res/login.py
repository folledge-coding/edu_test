...
class Login_req:
    def req(self):
        ...
        session = win32com.client.DispatchWithEvents("XA_Session.XASession", Login_res)
        session.ConnectServer(os.getenv('LS-URL'), 20001)
        session.Login(os.getenv('LS-ID'), os.getenv('LS-PASSWORD'), os.getenv('LS-CERT'), 0, False)
